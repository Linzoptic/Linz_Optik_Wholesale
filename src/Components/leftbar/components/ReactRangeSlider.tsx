...
const ReactRangeSlider = ({
  ...
  const onFinalChange = () => {
    setSearchParams({
      ...currentQueries,
      min_price: range[0].toString(),
      max_price: range[1].toString(),
    });
  };

  return (
    <>
      {homePageTexts && pricesRange[1].currentPrice !== pricesRange[0].currentPrice ? (
        <div className="mt-10 px-2 w-full">
          <div>
            <h1>{homePageTexts?.filter.price.description} </h1>
          </div>
          <div className="flex justify-between items-center my-4 flex-wrap">
            <div className="flex items-center my-2">
              <h2 className="border py-[5px] md:px-[10px] lg:px-[20px] ">
                {pricesRange[0].currentPrice}
              </h2>
            </div>
            <div className="flex items-center">
              <h2 className="border py-[5px] md:px-[10px] lg:px-[20px]">
                {pricesRange[1].currentPrice}
              </h2>
            </div>
          </div>
          <Range
            ...
            values={range}
            onChange={(values) =>
              setPricesRange((state) => {
                return state.map((elem: any, index) => ({
                  ...elem,
                  currentPrice: values[index],
                }));
              })
            }
        ...
        <div className="w-full bg-gray-300 animate-pulse h-[90px]"></div>
      )}
    </>
  );
};
